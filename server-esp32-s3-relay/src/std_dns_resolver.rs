...
impl Resolver for StdDnsResolver {
    ...
    fn lookup_ipv6(&self, host: String) -> BoxFuture<Result<BoxIter<Ipv6Addr>, DnsError>> {
        // IPv6 is nuked on this build (lwIP IPv6 off, IPv4-only network). Return no
        // addresses so the relay's happy-eyeballs dialer only attempts IPv4 — fewer
        // concurrent dials means a smaller FuturesUnorderedBounded buffer, which is
        // what was OOM'ing on the heap-tight no-SPIRAM S3.
        debug!("[std-dns] lookup_ipv6 {host}: IPv6 disabled");
