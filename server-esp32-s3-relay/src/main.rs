...
// under load. To profile, uncomment the `#[global_allocator]` below and tune
// `alloc_log::THRESHOLD`; run without `--monitor` so the console doesn't throttle it.
#[allow(dead_code)]
mod alloc_log;
...
fn main() {
    ...
    let rt = tokio::runtime::Builder::new_current_thread()
        ...
        // Relay path is deeper than direct QUIC; 4 KB is too tight if any relay/
        // DNS work lands on a blocking thread. The S3 has ~165 KB free heap here,
        // so spend some on stack headroom.
    ...
    rt.block_on(async {
        ...
        let mut builder = iroh::Endpoint::builder(iroh::endpoint::presets::Empty)
            ...
            .max_tls_tickets(0)
            // RELAY-ONLY variant: connections are carried over an n0 production
            // relay instead of direct IP. We pin a SINGLE relay rather than
            // RelayMode::Default (all 4 prod relays): the net_report reportgen
            // actor probes every relay in the map concurrently to pick the
            // closest, and racing all 4 at once blew the heap on the no-SPIRAM S3
            // (a ~12 KB alloc failed -> abort). One relay = no race = it fits.
            // Hard-coded to the EU relay; change the host for your region
            // (use1-1 / usw1-1 / euc1-1 / aps1-1 .relay.n0.iroh.link). The endpoint
            // still binds a UDP socket, but the ticket advertises only this relay.
            .relay_mode(iroh::RelayMode::Custom(iroh::RelayMap::from_iter([
                iroh::RelayConfig::from(
                    "https://euc1-1.relay.n0.iroh.link."
                        .parse::<iroh::RelayUrl>()
                        .expect("valid relay url"),
                ),
            ])))
            // KEEP the IP transport (deliberately NOT clear_ip_transports). On the
            // no-SPIRAM S3, relay selection via QAD — lightweight QUIC packets to
            // the relay, reusing the endpoint — is far cheaper than the HTTPS probe,
            // which drags in the full reqwest/hyper client (~85 KB → OOM). iroh only
            // runs QAD when an IP transport exists (it gates on has_ip_transports),
            // so we keep the IP transport for *selection* and make this "relay-only"
            // by advertising only the relay URL in the ticket (see below). A true
            // clear_ip_transports() build needs net_report skipped (pin the home
            // relay) to dodge the reqwest probe — that's an iroh-side change.
            .net_report_config({
                let mut c = iroh::NetReportConfig::default();
                // HTTPS probe OFF: reqwest/hyper is the ~85 KB heap hog. QAD (UDP,
                // over the kept IP transport) does relay selection instead. The
                // minimal crypto provider can't do real-cert HTTPS anyway.
        ...
        // DIAGNOSTIC (relay variant): the per-connection stack/heap prints never
        // fire because we crash during relay activation, before any connection.
        // Sample the main-task stack high-water + heap every 100 ms so the last
        // line before the crash shows the peak stack (to right-size the stack)
        // and the heap fragmentation as the relay goes active. Remove once tuned.
        tokio::spawn(async {
            loop {
                let stack_free =
                ...
                info!(
                    "[diag] stack_free={stack_free} of 90112  heap_free={heap_free}  largest_8bit={largest}"
                );
                tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
            }
        });

        // RELAY-ONLY: wait until the endpoint has registered with a home relay.
        // Until then endpoint.addr() carries no relay URL, so there would be
        // nothing dialable to put in the ticket. (If the relay can't be reached —
        // e.g. the minimal crypto provider can't complete the relay TLS — this
        // will block here, which is itself the diagnostic.)
        info!("Waiting for home relay...");
        endpoint.online().await;
        info!("Online — home relay established");

        let addr = endpoint.addr();
        let endpoint_id = addr.id;

        // Relay-only ticket: drop every direct IP address, keep only the relay
        // URL(s). The client dials this endpoint through the relay; no direct IP
        // transport is advertised.
        let mut relay_addr = addr.clone();
        relay_addr.addrs.retain(|a| a.is_relay());
        let relay_ticket = EndpointTicket::new(relay_addr);
        ...
        info!("Iroh endpoint bound (relay-only)");
        info!("  Device IP:    {wifi_ip}");
        info!("  Endpoint ID:  {endpoint_id}");
        info!("  Relay ticket: {relay_ticket}");
