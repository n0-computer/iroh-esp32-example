...
fn main() {
    ...
    rt.block_on(async {
        ...
        // Resolve our identity up front: building the mDNS lookup needs the endpoint
        // ID, and we want a handle to it (to subscribe to discovery events) before
        // the endpoint exists.
        let secret_key = parse_secret_key().unwrap_or_else(SecretKey::generate);
        let endpoint_id = secret_key.public();

        // Build the mDNS lookup ourselves instead of passing a builder, so we keep a
        // handle. MdnsAddressLookup is Clone and every clone shares one background
        // swarm-discovery actor, so the instance iroh drives (advertise/resolve) is
        // the same one we subscribe to below.
        let mdns = MdnsAddressLookup::builder()
            .build(endpoint_id)
            .expect("failed to build mDNS address lookup");

        // DIAGNOSTIC: dump every endpoint discovered on the LAN over mDNS. swarm-
        // discovery browses continuously, so any other iroh+mDNS peer announcing
        // `_irohv1._udp.local` shows up here (with its addresses), whether or not we
        // ever dial it — this is how we verify the *reception* side works.
        let mut discovered = mdns.subscribe().await;
        tokio::spawn(async move {
            while let Some(event) = discovered.next().await {
                info!("[mdns] discovery event: {event:?}");
            }
        });

        let builder = iroh::Endpoint::builder(iroh::endpoint::presets::Empty)
            ...
            // mDNS LOCAL DISCOVERY: advertise this endpoint's node ID + addresses
            // over multicast DNS on the LAN, and resolve peers by node ID. This is
            // what makes the SHORT ticket (node-id only) dialable on the LAN with no
            // server involved — just UDP multicast (swarm-discovery), no TLS.
            // Pass the pre-built `mdns` (see above) so we share its actor.
            .address_lookup(mdns)
            // Relay + pkarr too: SPIRAM has the heap to spare, so keep the n0 relay
            // (NAT traversal + off-LAN reachability) and pkarr DNS publishing
            // alongside mDNS. On the LAN mDNS resolves first; off-LAN the relay path
            // still works. This is the "have your cake and eat it" SPIRAM variant.
        ...
        // iroh's netwatch can't enumerate the WiFi interface address on esp-idf, so
        // its direct-address discovery stays empty. Relay still gives us a relay URL
        // to publish, but mDNS would only ever announce the relay path (or nothing
        // local) — peers on the LAN couldn't get a direct IP. Feed the DHCP-assigned
        // address in explicitly as a configured external address: it flows into
        // `direct_addrs`, so both pkarr and the mDNS lookup advertise a dialable LAN
        // address — which is what makes the SHORT ticket directly dialable on the LAN.
        endpoint
            .add_external_addr(std::net::SocketAddr::new(wifi_ip.into(), port))
            .await;

        // Short ticket: just the endpoint ID. With mDNS discovery enabled, this is
        // dialable on the LAN — the peer resolves the address over multicast DNS.
