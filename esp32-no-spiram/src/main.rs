...
// Allocation-profiling allocator. Enabled here to hunt the post-interaction OOM
// on this board — tune `alloc_log::THRESHOLD` to trade detail for log noise.
mod alloc_log;
...
impl ProtocolHandler for Echo {
    async fn accept(&self, connection: Connection) -> Result<(), AcceptError> {
        ...
        // 48 KB stack. tokio current_thread runs every async task here, so this is
        // the deepest the QUIC/TLS handshake + echo ever got. Whatever's left over
        // is dead weight we can return to the heap via CONFIG_ESP_MAIN_TASK_STACK_SIZE.
        let stack_free = unsafe {
            esp_idf_svc::sys::uxTaskGetStackHighWaterMark(core::ptr::null_mut())
        };
        info!("[stack] main task high-water: {stack_free} bytes free of 49152");
