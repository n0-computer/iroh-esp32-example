...
impl ProtocolHandler for Echo {
    async fn accept(&self, connection: Connection) -> Result<(), AcceptError> {
        ...
        // stack. tokio current_thread runs every async task here, so this is the
        // deepest the relay/pkarr setup + QUIC/TLS handshake + echo ever got.
        // Whatever's left over is headroom we could trim from
        // CONFIG_ESP_MAIN_TASK_STACK_SIZE and hand back to the internal-SRAM heap.
        // Reports "of <configured>" straight from the sdkconfig so it never goes stale.
