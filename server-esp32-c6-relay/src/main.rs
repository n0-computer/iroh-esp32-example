...
fn main() {
    ...
    rt.block_on(async {
        ...
        let mut builder = iroh::Endpoint::builder(iroh::endpoint::presets::Empty)
            ...
            // TLS session-resumption cache off — saves the hashbrown table; an
            // embedded server never resumes.
            .max_tls_tickets(0)
            // Relay connectivity, but pinned to ONE relay. RelayMode::Default probes
            // all 4 prod relays concurrently in net_report reportgen, and racing them
            // blew the heap on the no-PSRAM C6 (a 43-byte alloc failed -> abort) — the
            // same failure the no-SPIRAM S3 hit. One relay = no race = it fits. Change
            // the host for your region (use1-1 / usw1-1 / euc1-1 / aps1-1).
            ...
            // NO pkarr discovery. Publishing to n0 DNS is real-cert HTTPS, which the
            // minimal crypto provider (X25519 + AES-128-GCM, no RSA) cannot do — it
            // errored as `pkarr_publish` on this board. So this variant gives relay
            // connectivity (NAT traversal, dial over the relay) but NOT short-ticket
            // discovery; that needs the full-crypto SPIRAM target.
            //
            // HTTPS latency probes / captive-portal detection are off for the same
            // crypto reason; QAD (UDP) probes still measure relay latency.
            .net_report_config({
